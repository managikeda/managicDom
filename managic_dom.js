...
class Timeline {
  ...
  // ---- API ----
  moveTo(x,y,frames,easing){ return this._pushAct(this._tween({x,y}, frames, ['x','y'], easing)); }

  moveBy(dx,dy,frames,easing){
    const act = this._tween({}, frames, ['x','y'], easing);
  ...
  scaleTo(sx, sy, frames, easing){
    if (sy == null) sy = sx;
    return this._pushAct(this._tween({ scaleX:+sx||0, scaleY:+sy||0 }, frames, ['scaleX','scaleY'], easing));
  }

  rotateTo(deg,frames,easing){ return this._pushAct(this._tween({rotation:+deg||0}, frames, ['rotation'], easing)); }

  fadeTo(opacity,frames,easing){ return this._pushAct(this._tween({opacity:+opacity||0}, frames, ['opacity'], easing)); }
